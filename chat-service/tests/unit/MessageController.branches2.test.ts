...
describe('MessageController additional branches', () => {
  ...
  test('sendMessage - sender and receiver same triggers validation error', async () => {
    ...
    expect(next.mock.calls[0][0].message).toMatch(/Sender and receiver cannot be the same user/i)
  })
  ...
  test('sendMessage - empty message (only whitespace) returns friendly error', async () => {
    ...
    expect(next).toHaveBeenCalled()
    expect(next.mock.calls[0][0].message).toMatch(/Message cannot be empty/i)
  })
  ...
  test('sendMessage - message exceeds max length', async () => {
    ...
    expect(next).toHaveBeenCalled()
    expect(next.mock.calls[0][0].message).toMatch(/exceeds maximum length of 5000/i)
  })
  ...
  test('sendMessage - missing receiverId returns receiver required message', async () => {
    ...
    expect(next).toHaveBeenCalled()
    expect(next.mock.calls[0][0].message).toMatch(/Receiver ID is required/i)
  })
  ...
  test('fetchConversation - DB throws and returns 500', async () => {
    ...
    const req: any = { params: { receiverId: 'r2' }, user: { _id: 's1' }, query: {} }
    const res: any = { json: jest.fn(), status: jest.fn().mockReturnThis() }
    const next = jest.fn()
  ...
  test('markAsRead - missing senderId returns 400 message', async () => {
    ...
    expect(next).toHaveBeenCalled()
    expect(next.mock.calls[0][0].message).toMatch(/Sender ID is required/i)
  })
})
